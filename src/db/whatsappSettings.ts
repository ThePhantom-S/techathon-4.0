 ...
 * The WhatsApp access token is never stored here — it lives only in
 * process.env.WHATSAPP_ACCESS_TOKEN on the backend.
 */

const SETTINGS_FILE = path.join(process.cwd(), 'flowshield_whatsapp_settings.json');
...
const DEFAULT_SETTINGS: WhatsAppAlertSettings = {
...
export function getWhatsAppAlertSettings(): WhatsAppAlertSettings {
  try {
...
export function saveWhatsAppAlertSettings(partial: Partial<WhatsAppAlertSettings>): WhatsAppAlertSettings {
  const current = getWhatsAppAlertSettings();
  const next: WhatsAppAlertSettings = {
  ...
  try {
    ...
    console.error('Error writing WhatsApp alert settings:', e);
  }
  return next;
}
