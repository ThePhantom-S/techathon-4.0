...
export const DashboardAnalytics: React.FC<DashboardAnalyticsProps> = ({
  ...
  // Build monthly cashflow bars from dailyPoints
  const monthlyData = [
  ...
  simulationResult.dailyPoints.forEach((pt) => {
    const monthNum = parseInt(pt.date.split('-')[1], 10); // 10=Oct, 11=Nov, 12=Dec
    const idx = monthNum - 10;
    if (idx >= 0 && idx < 3) {
  ...
  const recentActivity = [
    ...simulationResult.driverAnalysis.topOutflows.slice(0, 3).map((d) => ({
      label: d.entity,
    ...
    ...simulationResult.driverAnalysis.topInflows.slice(0, 3).map((d) => ({
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className={`p-5 rounded-2xl border ${isLight ? 'bg-[#FAFAFA] border-[#EAEAEA]' : 'bg-[#0A0A0A] border-[#222222]'}`}>
              ...
              <span className="text-xs text-[#A1A1AA] mt-1 block">500 Monte Carlo Runs</span>
            </div>
            <div className={`p-5 rounded-2xl border ${isLight ? 'bg-[#FAFAFA] border-[#EAEAEA]' : 'bg-[#0A0A0A] border-[#222222]'}`}>
