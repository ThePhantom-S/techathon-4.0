...
I can answer questions regarding your liquidity risk, driver analysis, or counterfactual strategies grounded strictly in verified engine numbers. What would you like to explore?`
          : `**FlowShield AI Chat is currently inactive.**

No active AI API key was found in your environment or Settings. To ensure high-precision financial analysis and prevent simulated output, AI Chat only functions when an API key is connected.

To activate AI Chat, please provide your **Gemini**, **Groq**, or **OpenRouter** API key in Platform **Settings > Integrations**.`,
  ...
  const handleSendQuery = async (queryText?: string) => {
    ...
    // AI Chat only works when API keys are present
    if (!isAiConfigured) {
      const disabledMsg: Message = {
        id: `ai-${Date.now()}`,
        sender: 'ai',
        text: 'AI Chat is disabled because no active API key was found. Please configure your Gemini, Groq, or OpenRouter API key in Platform Settings > Integrations to enable live AI responses.',
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
      };
        ...
        <div className={`p-3.5 border-t ${
          ...
          >
            <input
              ...
              placeholder={isAiConfigured ? "Ask about cash flow, risks, or strategy..." : "AI Chat requires an active API key in Settings..."}
            ...
            <button
              type="submit"
              disabled={!isAiConfigured || loading || !inputQuery.trim()}
