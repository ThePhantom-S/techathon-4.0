...
interface ZohoBooksConnectorProps {
  onBack: () => void;
...
const REDIRECT_URI = typeof window !== 'undefined' ? `${window.location.origin}/api/connect/zoho/callback` : '';

export const ZohoBooksConnector: React.FC<ZohoBooksConnectorProps> = ({ onBack, onConnected }) => {
  ...
  // Validate form
  const isFormValid = clientId.trim().length > 5 && clientSecret.trim().length > 5 && orgId.trim().length > 0;

  const handleStartOAuth = useCallback(async () => {
    if (!isFormValid) {
      setError({ code: 'MISSING_FIELDS', message: 'Please fill in all required fields.' });
      setStep('ERROR');
      return;
    }
    ...
    try {
      // Store credentials server-side and get auth URL
      const res = await fetch('/api/connect/zoho/authorize', {
        method: 'POST',
      ...
      setLoadingMessage('Redirecting to Zoho authorization page...');
  ...
  }, [clientId, clientSecret, orgId, isFormValid]);

  const handleSync = useCallback(async () => {
    setStep('CONNECTING');
    setLoadingMessage('Syncing financial data from Zoho Books...');
    try {
      const res = await fetch('/api/connect/zoho/sync', { method: 'POST' });
      const data = await res.json();
  ...
  const handleDisconnect = useCallback(async () => {
    try {
      await fetch('/api/connect/zoho/disconnect', { method: 'POST' });
      ...
      <div className="relative">
        <input
          ...
          className={`w-full pr-20 p-2.5 rounded-xl font-sans text-xs border focus:outline-none transition-colors ${
            isLight
              ? 'bg-slate-50 border-slate-200 focus:border-orange-500 text-slate-900 placeholder:text-slate-400'
              : 'bg-zinc-900 border-zinc-800 focus:border-orange-500 text-zinc-100 placeholder:text-zinc-500'
          }`}
        />
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-4">
        <div className={`rounded-2xl max-w-md w-full p-8 text-center space-y-6 shadow-2xl border ${
          ...
          <div className="relative mx-auto w-16 h-16">
            ...
            <Zap className="w-6 h-6 text-orange-400 absolute inset-0 m-auto animate-pulse" />
          </div>
          <div>
            ...
            <p className="text-xs text-orange-500 font-mono mt-2 animate-pulse">{loadingMessage}</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-4">
        <div className={`rounded-2xl max-w-lg w-full p-6 space-y-5 shadow-2xl border ${
          ...
          <div className="flex justify-center gap-3 pt-2">
            ...
            <button
              onClick={onBack}
              className="px-4 py-2 rounded-xl bg-orange-500 hover:bg-orange-600 text-white font-bold font-mono text-xs cursor-pointer"
            >
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-4">
        <div className={`rounded-2xl max-w-lg w-full p-6 space-y-5 shadow-2xl border ${
          ...
          <div className={`rounded-xl p-4 space-y-2 font-mono text-xs border ${
            ...
            {status.organizationName && (
              <div className="flex justify-between">
                <span className={isLight ? 'text-slate-500' : 'text-white/50'}>Organization</span>
                <span className={`font-bold ${isLight ? 'text-slate-900' : 'text-white'}`}>{status.organizationName}</span>
              </div>
            )}
            {status.lastSynced && (
              <div className="flex justify-between">
            ...
            {status.currentCash !== undefined && (
              <div className="flex justify-between">
                <span className={isLight ? 'text-slate-500' : 'text-white/50'}>Cash Balance</span>
          ...
          <div className="flex gap-3">
            <button
              onClick={onBack}
              className="flex-1 py-2.5 rounded-xl bg-orange-500 hover:bg-orange-600 text-white font-bold font-mono text-xs uppercase tracking-wider cursor-pointer"
            >
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // RENDER: CONNECTED STATE (already authorized, show status)
  // ══════════════════════════════════════════════════════════════════════════
  if (step === 'CONNECTED') {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-4">
        <div className={`rounded-2xl max-w-lg w-full p-6 space-y-5 shadow-2xl border ${
          ...
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-orange-500 flex items-center justify-center text-white font-black text-sm shadow-lg shadow-orange-500/30">
              Z
            </div>
            <div>
          ...
          <div className={`flex items-center gap-3 p-3 rounded-xl border ${
            ...
            <div className={`text-xs font-mono ${isLight ? 'text-emerald-700' : 'text-emerald-400'}`}>
              Your Zoho Books data is synchronized and available for liquidity forecasting.
            </div>
          </div>
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // RENDER: CONFIGURE (main form) / GUIDE
  // ══════════════════════════════════════════════════════════════════════════
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-4">
      <div className={`rounded-2xl max-w-2xl w-full p-6 space-y-4 shadow-2xl border relative max-h-[85vh] overflow-y-auto font-sans ${
        ...
        <div className={`rounded-xl p-3.5 border ${
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 text-xs">
            {[
              'Read-only access',
              'No payment or transaction authority',
              'Your accounting password is never shared',
              'Only required financial data is accessed',
          ...
          {showGuide && (
            <div className={`p-4 space-y-4 border-t text-xs font-mono ${
              ...
              <div className={`p-3 rounded-lg border ${
                isLight ? 'bg-orange-50 border-orange-200 text-orange-700' : 'bg-orange-500/10 border-orange-500/30 text-orange-300'
              }`}>
                <strong>View Setup Guide — Zoho API Console</strong>
              </div>

              {[
                { step: 1, text: 'Open the Zoho API Console at https://api-console.zoho.in/' },
                { step: 2, text: 'Create a new Server-based Application under "Client Apps"' },
                { step: 3, text: `Add FlowShield\'s Redirect URI: ${REDIRECT_URI}` },
                { step: 4, text: 'Configure the required READ-only Zoho Books scopes (see below)' },
                { step: 5, text: 'Copy the Client ID and Client Secret from the app settings' },
                { step: 6, text: 'Click "Connect & Authorize" below to start the OAuth flow' },
              ].map(({ step: s, text }) => (
                <div key={s} className="flex gap-3">
                  <div className="w-5 h-5 rounded-full bg-orange-500 text-white flex items-center justify-center text-[10px] font-bold shrink-0 mt-0.5">
                    {s}
                  </div>
        ...
        <div className="space-y-3">
          ...
          <TextField
            ...
            hint="Found in Zoho API Console → Client Apps"
          />
          ...
          <TextField
            ...
            hint="Found in Zoho Books → Settings → Organization Profile"
          />
          ...
          <div className={`flex items-center gap-2 p-2.5 rounded-xl text-xs ${
            ...
            <span>FlowShield will never ask for your Zoho username, password, bank PIN, or OTP.</span>
          </div>
        </div>
